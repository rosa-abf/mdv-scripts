...
echo '--> mdv-scripts/build-packages/cooker: openmandriva-arm64.sh'
...
sudo sh -c "echo ':aarch64:M::\x7fELF\x02\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\xb7:\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xfe\xff\xff:/usr/bin/qemu-arm64-static:' > /proc/sys/fs/binfmt_misc/register"
...
(while [ ! -e  $tmpfs_path/openmandriva-2013.0-$platform_arch/root/usr/bin/ ]
  do sleep 1;done
  sudo cp $rpm_build_script_path/cooker/qemu-arm64-static  $tmpfs_path/openmandriva-2013.0-$platform_arch/root/usr/bin/) &
  subshellpid=$!
