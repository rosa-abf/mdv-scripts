...
if [ $cpu != "aarch64" ] ; then
   # wipe old-style string from env
   # comment this string after all
...
(while [ ! -e  $tmpfs_path/openmandriva-$platform_arch/root/usr/bin/ ]
  do sleep 1;done
  sudo cp $rpm_build_script_path/qemu-aarch64*  $tmpfs_path/openmandriva-$platform_arch/root/usr/bin/) &
  subshellpid=$!
...
python2 $rpm_build_script_path/check_arch.py $src_rpm_name "$platform_arch"
rc=$?
if [ $rc != 0 ] ; then
